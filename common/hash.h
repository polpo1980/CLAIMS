...
class PartitionFunction {
 public:
  enum PartitionStyle { hash_f, range_f, round_robin_f };
  /**
  ...
  virtual PartitionStyle getPartitionFashion() const = 0;
  /**
 ...
 protected:
  int min_, max_;
  unsigned long int
      k_; /**< \f$ \_k=log_2(k) \f$, where \f$k\f$ is number of buckets */
...
class RoundRobinPartitionFunction : public PartitionFunction {
 public:
  ...
  inline unsigned int getNumberOfPartitions() const { return range_; }
  PartitionStyle getPartitionFashion() const;
...
/**
 * This class is used to achieve range partition, where each range is of equal
 * size.
...
class UniformRangePartitionFunction : public PartitionFunction {
 public:
  ...
  PartitionStyle getPartitionFashion() const;
  unsigned getNumberOfPartitions() const { return 1 << k_; }
...
class GeneralModuloFunction : public PartitionFunction {
 public:
  ...
  GeneralModuloFunction(const int& range, const int& min, const int& skipbits)
      : PartitionFunction(min, -1, range * (1 >> 12)),
        range_(range),
        skipbits_(skipbits) {}
  ~GeneralModuloFunction(){};
  inline unsigned int get_partition_value(const int& value) const {
  ...
  PartitionStyle getPartitionFashion() const;
  unsigned getNumberOfPartitions() const { return range_; }
...
class BoostHashFunction : public PartitionFunction {
 public:
  BoostHashFunction(){};
  BoostHashFunction(const int& range)
  ...
  PartitionStyle getPartitionFashion() const { return hash_f; }
  unsigned getNumberOfPartitions() const { return range_; }
...
class PartitionFunctionFactory {
 public:
  ...
  static PartitionFunction* createGeneralModuloFunction(
      const int& range, const int& min = 0, const int& skipbits = 0);
  /* The range of ModuloFunction should be a power of 2. If you want to use
   * other range, you should consider
   * to use GeneralModuloFunction. */
  static PartitionFunction* createModuloFunction(const int& range,
                                                 const int& min = 0,
                                                 const int& skipbits = 0);
  static PartitionFunction* createUniformRangeHashFunction(const int& range,
                                                           const int& min,
                                                           const int& max);
  ...
  static PartitionFunction* createBoostHashFunction(const int& range) {
    return new BoostHashFunction(range);
  }
};
